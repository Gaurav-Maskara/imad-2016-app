...
<div class="row social">
  <ul class="social-icons">
    ...
    <li><a href="https://github.com/Gaurav-Maskara/imad-2016-app" target="_blank"><img src="img/git.jpg" alt="git"></a></li>
  </ul>
</div>
<div class="footer">
  <div class="container">
...
app.get('/get-comments/:articleName', function (req, res) {
   // make a select request
   // return a response with the results
   pool.query('SELECT newcomment.*, "user".username FROM article, newcomment, "user" WHERE article.title = $1 AND article.id = newcomment.article_id AND newcomment.user_id = "user".id ORDER BY newcomment.timestamp DESC', [req.params.articleName], function (err, result) {
      if (err) {
          res.status(500).send(err.toString());
      } else {
          res.send(JSON.stringify(result.rows));
        ...
        pool.query('SELECT * from article where title = $1', [req.params.articleName], function (err, result) {
            if (err) {
                ...
                if (result.rows.length === 0) {
                    ...
                    pool.query(
                        "INSERT INTO newcomment (comment, article_id, user_id) VALUES ($1, $2, $3)",
